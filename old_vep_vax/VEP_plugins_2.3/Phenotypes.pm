...
use VAX qw(get_tva_info get_unique lstrip trim);
...
sub run {
    ...
    if(defined($tva_info{translation})
       && defined($tva_info{ensg})
       && defined($tva_info{ensp})
       && defined($tva_info{altered_aa_start})
       && defined($tva_info{altered_aa_end})
       && defined($tva_info{genomic_coords})
       ){
        ...
        my $aligned_model_orthologue_proteins =
            get_model_orthologous_residues(
                ...
                $tva_info{ensg},
                $tva_info{ensp},
                $tva_info{altered_aa_start},
                $tva_info{altered_aa_end}
            );
        ...
        if (defined($params{model})) {
            my ($model_phenotype_annotations, $cosmic_model_phenotype_annotations, $hgmd_public_model_phenotype_annotations) =
                get_model_phenotypes(
                    $self,
                    $tva_info{genomic_coords},
                    $tva_info{ensp},
                    $tva_info{altered_aa_start},
                    $tva_info{altered_aa_end},
                    $tva_info{aligned_model_orthologue_proteins}
                    );
                ...
                )
            {
                ...
                my $phenotypes_locus = join( '|', @{$model_phenotype_annotations});
                push(@DISEASES_PHENOTYPES, @{$model_phenotype_annotations});
...
#returns an array of arrays of phenotypes associated with model genomic coordinates
    ...
    foreach my $vf ( @{$vfs} ) {
        ...
        my ($phenotypes_vas, $cosmic_phenotypes_vas, $hgmd_public_phenotypes_vas)
            = get_phenotypes_from_variant_annotations($vas);
        if (scalar(@{$phenotypes_vas}) > 0) {
                @phenotypes = (@phenotypes, @{$phenotypes_vas});
        }
        if (scalar(@{$cosmic_phenotypes_vas}) > 0) {
                @cosmic_phenotypes = (@cosmic_phenotypes, @{$cosmic_phenotypes_vas});
        }
        if (scalar(@{$hgmd_public_phenotypes_vas}) > 0) {
                @hgmd_public_phenotypes = (@hgmd_public_phenotypes, @{$hgmd_public_phenotypes_vas});
        }
    }
