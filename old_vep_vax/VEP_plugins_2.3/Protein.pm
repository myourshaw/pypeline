...
use VAX qw(get_tva_info get_consequence_info complement get_unique replace_base replace_str overlaps);
...
sub run {
    ...
    if(defined($tva_info{translation})
       && defined($tva_info{protein_sequence})
       && defined($tva_info{transcript})
       ){
        $line_hash->{Protein_Length} = $tva_info{protein_length};
        ...
        my ($protein_sequence_gained, $protein_length_increase) =
            determine_effect_of_stop_lost_on_protein(
                $consequences_info{consequences_ranks_so},
                $tva_info{protein_sequence},
                $tva_info{altered_base_start},
                $tva_info{transcript},
                $tva_info{amino_acid_variant} #$transcript_snp_reads
                );
        #Protein_Length_Increase
    ...
    if(defined($tva_info{transcript})){
        ...
        my ($reference_splice_site, $variant_splice_site) =
            determine_effect_of_essential_splice_site_on_splice_site(
                ...
                $tva_info{reference_allele}, #$Chromosome_Reference
                $tva_info{non_reference_allele}
                );
        #Reference_Splice_Site
