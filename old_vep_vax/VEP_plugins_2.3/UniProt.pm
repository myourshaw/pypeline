...
=head1 SYNOPSIS
 ...
 perl variant_effect_predictor.pl -i variations.vcf --plugin UniProt
...
=head1 DESCRIPTION
 ...
 ENZYME_REGULATION, FUNCTION, GeneNames, GO, GO_term, INDUCTION, INTERACTION, KEGG,
 KW, MIM_gene, MIM_phenotype, MISCELLANEOUS, PATHWAY, Pathway_Interaction, PE, POLYMORPHISM,
...
sub run {
    ...
    if(defined($tva_info{enst})){
        my $uniprot = get_uniprot_from_vw($tva_info{enst});
    ...
    if(defined($tva_info{translation})
       ...
       && defined($tva_info{altered_aa_end})
        ...
        my $uniprot_feature = get_uniprot_feature_from_vw($tva_info{enst}, $tva_info{altered_aa_start}, $tva_info{altered_aa_end});
