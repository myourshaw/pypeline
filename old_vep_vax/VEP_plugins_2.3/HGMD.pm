...
sub run {
    ...
    my $hgmd_chromosome = $tva_info{chrom};
    my $hgmd_coordSTART = $tva_info{chrom_start};
    my $hgmd_coordEND = $tva_info{chrom_start}<=$tva_info{chrom_end} ? $tva_info{chrom_end} : $tva_info{chrom_start}+1;
    my $hgmd_strand = $tva_info{chrom_strand}==1 ? '+' : '-';
    ...
    if (defined $tva_info{hgnc}){
        ...
        my $query = "CALL $database.gene2hgmd_disease('$tva_info{hgnc}')";
    ...
    my $query = "CALL $database.coords2hgmd('$hgmd_chromosome',$hgmd_coordSTART,$hgmd_coordEND)";
    my $qh = $self->{conn}->prepare($query);
    $qh->execute() or die "Unable to execute $query: $DBI::errstr\n";
    while (my @row = $qh->fetchrow_array()){
