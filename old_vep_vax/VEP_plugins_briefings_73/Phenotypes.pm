...
#returns array of arrays of phenotypes associated with gene
    ...
    return get_phenotypes_from_phenotype_featuress($pfs);
}
    ...
    foreach my $vf ( @{$vfs} ) {
        ...
        if(ref($variation) && $variation->isa('Bio::EnsEMBL::Variation::Variation')) {
            ...
            my ($phenotypes_pfs, $cosmic_phenotypes_pfs, $hgmd_public_phenotypes_pfs)
                = get_phenotypes_from_phenotype_featuress($pfs);
...
sub get_phenotypes_from_phenotype_featuress{
...
#phenotype information for (@phenotypes, @cosmic_phenotypes, @hgmd_public_phenotypes)
    ...
    foreach my $pf (@{$pfs}) {
        ...
        if (defined($pf->source()) && $pf->source() eq 'COSMIC'){
            if (defined($phenotype->description()) && defined( $pf->variation_names())) {
                push(@{$cosmic{lstrip(lstrip(trim($phenotype->description()),'COSMIC:'),'tumour_site:')}}, trim($pf->variation_names()));
            ...
            if (defined( $pf->variation_names())) {
                push(@hgmd_public_phenotypes, trim($pf->variation_names()));
            }
        }
        else{
            my $key = undef;
