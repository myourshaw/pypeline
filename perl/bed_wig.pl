            ...
            if($found =~ /$mismatches_regex/i && !$opts{b}){
                ...
                while(<B>){
                    if(/$bed_regex/i){
                        ...
                        my $score = $11;
                        my $strand = $12;
                        my $bed_key = "$params{blat}|$params{flowcell}|$lane|$tag|$chrom|$chromStart";
                        printf BED ("%s\t%s\t%s\t%s\t%s\t%u\t%u\t%s\t%s\t%s\t%s\t%u\t%u\t%u\t%u\t%u\n")
                        ,$params{blat},$params{flowcell},$lane,$tag
                ...
                while(<W>){
                    if(/$wig_regex/i){
                        ...
                        printf WIG ("%s\t%s\t%s\t%s\t%s\t%u\t%u\t%u\n"
                                    ,$params{blat},$params{flowcell},$lane,$tag,$chrom,$chromStart,$chromEnd,$reads);
                    }
                }
            }
        }
