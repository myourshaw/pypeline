...
# perl ~/Lab/pipeline/count_tag_reads.pl ~/Lab/Autism/"MYO1D sequencing"/solexa/MYO1Dreadcounts.job ~/Lab/Autism/"MYO1D sequencing"/solexa/MYO1Dreadcounts.txt
...
while(<>){
    ...
    if($_){
        ...
        #flowcell and lanes
            ...
            print "looking for cell $1 lanes ".join(", ",@lanes)."\n";
			use File::Find;
			find sub{
				if(/$seqfileregex/i){
					...
					while(<SEQFILE>){
						...
						if(/$params{seq_line}/i){
							...
							print STDERR "unrecognized sequence in $seqfile\n\t$_\n";
						}
					}
				}
			}, $dir;
