...
my %params = (
	...
	create_sequence_file => 0, #bool (default = 0)
	...
	use_ensembl_human => 1,
	use_ensembl_vega => 1,
	include_splice_site_acceptor_bases => 2, #YNYYRAYnNYAG upstream default 2
	include_splice_site_donor_bases => 2, #GTRAGT downstream default 2
	merge_intervals => 1 #bool default 1
...
sub gene {
	...
	if($params{include_gene_upstream_promoter_region}
			&& $params{separate_records_for_promoter_upstream_downstream_regions}
			&& $gene_extra_up){
		print $gene->start;
		print_slice(
			$gene_name.UPSTREAM,
