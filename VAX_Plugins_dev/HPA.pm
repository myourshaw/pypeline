...
sub new {
    ...
    ($self->{hpa_version}) = @{$self->{params}} ? @{$self->{params}} : '';
...
sub get_header_info {
    my ($self) = @_;
    my $hpa_version = $self->{hpa_version};

    my %tissue_cell_type_columns;
    $tissue_cell_type_columns{HPA_subcellular_location} = "Subcellular localisation of proteins based on immunofluorescently stained cells (Human Protein Atlas $hpa_version)";
    my @output_cols = ();
    ...
    while (my @row = $qh->fetchrow_array()){
        push @output_cols, 'HPA_'.unspace($row[0]).'_'.unspace($row[1]);
    ...
    @output_cols = sort(@output_cols);
    push @output_cols, 'HPA_subcellular_location';
