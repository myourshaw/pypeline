...
sub new {
    ...
    ($self->{mgi_version}) = @{$self->{params}} ? @{$self->{params}} : '';
...
sub get_header_info {
    my $self = shift;
    my $mgi_version = $self->{mgi_version};
    ...
    return {
            MOUSE_PHENOTYPES => "Phenotypes associated with gene knockout mice (JAX MGI $mgi_version)",
        };
}
