...
=head1 SYNOPSIS
 ...
 perl variant_effect_predictor.pl -i variations.vcf --plugin vw[,host,port,user,password,mysql,database,vax_database] --plugin HGMD,HGMD,[,host,port,user,password,mysql,database]
...
=head1 PARAMETERS
    ...
    hgmd_version (default: HGMD),host (default: cortex.local),port (default: 3306),user (default: vax),password (default: vax),platform (default: mysql),database (default: hgmd_pro)
...
sub new {
    ...
    my ($hgmd_version,$host,$port,$user,$password,$platform,$database) = @{$self->{params}};
    $hgmd_version ||= 'HGMD';
    $host ||= 'cortex.local';
    $port ||= 3306;
    $user ||= 'vax';
    ...
    $platform ||= 'mysql';
    $database ||= 'hgmd_pro';
    $self->{params} = [$hgmd_version,$host,$port,$user,$password,$platform,$database];
    $self->{hgmd_version} = $hgmd_version;
    
    #stored procedure now lives in vax database, no need for a connection to HGMD database
...
sub get_header_info {
    my $self = shift;
    my $hgmd_version = $self->{hgmd_version};
...
sub run {
    ...
    if (defined $bvfoa_info{hgnc}){
        ...
        my $query = "CALL $vw::vw_database.gene2hgmd_disease('$bvfoa_info{hgnc}')";
    ...
    my $query = "CALL $vw::vw_database.coord2hgmd('$hgmd_chromosome',$hgmd_coordSTART,$hgmd_coordEND)";
    my $qh = $vw::vw_conn->prepare($query);
    $qh->execute() or die "Unable to execute $query: $DBI::errstr\n";
    while (my @row = $qh->fetchrow_array()){
