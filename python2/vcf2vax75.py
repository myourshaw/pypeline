...
def run(vcfs,
    ...
    for vcf in vcfs:
        ...
        table = os.path.basename(vcf)
        my.print_log('Scheduling job to create sql scripts for {}.'.format(vcf))
        cmd= '{} {} --input {} database {} schema {} table {} clustered_index CHROM POS REF ALT --indexes ID'.format(
            config.get('DEFAULT','python'), sql_columns_py, vcf, 'vaxdatabase', 'vaxschema', table)
        job_name = 'sql_{}'.format(table)
        job = my.run_job(cmd, job_name, job_dir)
        sql_vcf_job_id = job.jobId
        job_ids.append(job.jobId)
        my.print_log('Scheduled job {} to create sql scripts for {}.'.format(job.jobId, vcf))
        
        if not no_vex:
            table = os.path.basename(vex)
            my.print_log('Scheduling job to create sql scripts for {}.'.format(vex))
            cmd= '{} {} --input {} database {} schema {} table {} clustered_index CHROM POS REF ALT --indexes ID'.format(
                config.get('DEFAULT','python'), sql_columns_py, vex, 'vaxdatabase', 'vaxschema', table)
            job_name = 'sql_{}'.format(table)
            job = my.run_job(cmd, job_name, job_dir)
            sql_vex_job_id = job.jobId
            job_ids.append(job.jobId)
            my.print_log('Scheduled job {} to create sql scripts for {}.'.format(job.jobId, vex))
        ...
        if  not no_gtx and has_genotypes:
            ...
            my.print_log('Scheduling job to create sql scripts for {}.'.format(gtx))
            cmd= '{} {} --input {} database {} schema {} table {} clustered_index CHROM POS REF ALT SAMPLE --indexes ID'.format(
                config.get('DEFAULT','python'), sql_columns_py, gtx, 'vaxdatabase', 'vaxschema', table)
            job_name = 'sql_{}'.format(table)
            job = my.run_job(cmd, job_name, job_dir)
            sql_gtx_job_id = job.jobId
            job_ids.append(job.jobId)
            my.print_log('Scheduled job {} to create sql scripts for {}.'.format(job.jobId, gtx))
