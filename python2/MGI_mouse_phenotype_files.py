...
def run(HMD, VOC):
    ...
    print 'read {} input lines, {} data input lines\nwrote {} data output lines to {}'.format(line_count, data_input_line_count, data_output_line_count, HMD)
    ...
    with open(VOC) as rpt, open(fixed_rpt, 'w') as fixed:
        ...
        for line in rpt:
            ...
            if not bool(line.strip()) or line.startswith('#'):
                ...
                fixed.write(output_line)
                data_output_line_count += 1
    print 'read {} input lines, {} data input lines\nwrote {} data output lines to {}'.format(line_count, data_input_line_count, data_output_line_count, VOC)
