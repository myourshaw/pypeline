...
def run(vcfs,
        max_part_records=10000, no_gtx=False, no_vex=False, no_vep=False,
    ...
    for vcf in vcfs:
        ...
        vex_metadata = [
        '##INFO=<ID=ALT,Number=1,Type=String,Description="Original VCF file\'s ALT field, which may contain multiple alleles">',
        '## VEX file created at {} by {} version {}. Copyright {}.'.format(run_time, name, version, copyright),
        '## VEX file has one line per ALT allele and omits any FORMAT and genotype columns',
        '## VEX --vcf {}'.format(vcf),
        '## VEX --vex {}'.format(vex),
        ]
        gtx_metadata = [
        '##INFO=<ID=ALT,Number=1,Type=String,Description="Original VCF file ALT field, which may contain multiple alleles">',
            ...
            for line in vcf_in:
                ...
                if line.startswith('##'):
                    ...
                    for this_alt in alt_list:
                        ...
                        vex_data_out['INFO'] = 'ALT={};{}'.format(data_in['ALT'], data_in['INFO'])
                        if not no_vex:
                            vex_out.write('\t'.join([vex_data_out[c] for c in vex_columns])+'\n')
                            vex_record_count += 1
                            accumulate_sql_spec(vex_data_out, vex_columns_spec)
                        if not no_vep and max_part_records != None:
                            if vex_data_out['#CHROM'] != this_chrom or (max_part_records != 0 and this_part_record_count >= max_part_records):
                                if this_part_fh:
                                    this_part_fh.close()
                    ...
                    if has_genotypes and not no_gtx:
                        ...
                        if format_split[0] == 'GT':
                            ...
                            for this_sample in sample_columns:
                                ...
                                gtx_data_out['INFO'] = 'ALT={};SAMPLE={};{}'.format(data_in['ALT'], this_sample.translate(info_translation), data_in['INFO'])
                                ...
                                if allele1 != '.' and allele2 != '.':
                                    ...
                                    gtx_data_out['AD1'] = ad[0] if ad and len(ad) > 0 else ''
                                    gtx_data_out['AD2'] = ad[1] if ad and len(ad) > 1 else ''
...
def main():
    ...
    #vcf2vex parameters
    parser.add_argument('--max_part_records', type=int, default=10000,
        help='maximum number of records per vex chromosome part file; if None do not create parts; if 0 split on chromosome only (default:None)')
    ...
    job_ids = run(vcfs=args.vcfs,
        max_part_records=args.max_part_records, no_gtx=args.no_gtx, no_vex=args.no_vex, no_vep=args.no_vep,
