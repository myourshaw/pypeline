...
#-t EstimateLibraryComplexity --job_dir /scratch1/tmp/myourshaw/hlee/jobs/bam_metrics_ayZJM9 --picard_args INPUT=/scratch0/tmp/aliz/bipolar/data/truseqABCD_myourshaw/bams/sample_bams/8000350018.sample.recalibrated.bam OUTPUT=/scratch1/tmp/myourshaw/hlee/metrics/bam_metrics/sample_metrics/8000350018.sample.recalibrated.bam.metrics.librarycomplexity READ_NAME_REGEX="'[^_]+_[^_]+_[0-9]+_([0-9]+)_([0-9]+).?[0-9]*_([0-9]+).*'" OPTICAL_DUPLICATE_PIXEL_DISTANCE=100

def _picard_info(reference=None, read_name_regex=None):
    ...
    if not read_name_regex:
        read_name_regex = config.get('picard', 'READ_NAME_REGEX')
    return {
    ...
    'MergeSamFiles': {'defaults': {'OUTPUT': None, 'CREATE_INDEX': 'true', 'CREATE_MD5_FILE': 'true', 'SORT_ORDER': 'coordinate', 'ASSUME_SORTED': 'false', 'MERGE_SEQUENCE_DICTIONARIES': 'false', 'USE_THREADING': 'true',},
        'run_parameters': {'processors': 8, 'memory': '6G'}},
    'MarkDuplicates': {'defaults': {'INPUT': None, 'OUTPUT': None, 'CREATE_INDEX': 'true', 'CREATE_MD5_FILE': 'true', 'REMOVE_DUPLICATES': 'false', 'MAX_FILE_HANDLES_FOR_READ_ENDS_MAP': 8000, 'READ_NAME_REGEX': '"'+read_name_regex+'"', 'OPTICAL_DUPLICATE_PIXEL_DISTANCE': 100, 'METRICS_FILE': None,},
        'run_parameters': {'processors': 8, 'memory': '6G'}},
    ...
    'CollectGcBiasMetrics': {'defaults': {'INPUT': None, 'OUTPUT':None, 'REFERENCE_SEQUENCE': reference, 'CHART_OUTPUT':None, 'SUMMARY_OUTPUT':None,},
        'run_parameters': {'processors': 8, 'memory': '6G'}},
    'EstimateLibraryComplexity': {'defaults': {'INPUT': None, 'OUTPUT':None, 'READ_NAME_REGEX': '"'+read_name_regex+'"', 'OPTICAL_DUPLICATE_PIXEL_DISTANCE':100,},
        'run_parameters': {'processors': 8, 'memory': '6G'}},
}

def run(tool, picard_args, multi_valued_arg_strings=None, redirect_stdout=None, job_dir=None,
        reference=None, read_name_regex=None, picard_current=None, email=None, synchronous=False, hold_jid=None, log_file=None):
    config = my.get_config()
    if log_file:
        logging.basicConfig(filename=log_file, format='%(asctime)s\t%(message)s', level=logging.DEBUG)
    picard_info = _picard_info(reference, read_name_regex)
...
def main():
    ...
    parser.add_argument('--tool', '-t', required=True, choices=_picard_info().keys(),
    ...
    parser.add_argument('--read_name_regex',
        help='read name regex (default: from config->picard.READ_NAME_REGEX)')
    ...
    job = run(tool=args.tool, picard_args=picard_args, multi_valued_arg_strings=args.multi_valued_arg_strings,
        ...
        hold_jid=args.hold_jid, log_file=args.log_file)
