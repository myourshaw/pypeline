...
class Vcf2VepError(Exception): pass
...
def main():
	...
	parser.add_argument('--input', '-i', nargs='+',
											help='input vcf file(s); output will be <vcf>.vep')
	parser.add_argument('--vep',
											help='path to Variant Effect Predictor perl executable')
	...
	if not args.vep:
		args.vep = config.get('ensembl','variant_effect_predictor')
	...
	for vcf in vcfs:
		...
		vep_out = os.path.join(output_dir, vcf_base+'.vep')
...
--hgnc \
--buffer_size 5000 \
'.format(args.vep, part, part+'.vep', vep_cache))
